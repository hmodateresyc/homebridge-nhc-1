...
class NikoClient {
    ...
    _connectEventSocket() {
        ...
        socket.on('data', chunk => {
            ...
            lines.forEach(line => {
                if (!line) return;
                if (this._log) this._log(`[NHC debug] event raw: ${line}`);
                try {
                    const parsed = JSON.parse(line);
                    // Controller sends either a plain object or an array of objects
                    const messages = Array.isArray(parsed) ? parsed : [parsed];
                    messages.forEach(msg => {
                        const key = msg && (msg.event || msg.cmd);
                        if (this._log) this._log(`[NHC debug] event: ${key} | allowed: ${key && ALLOWED_EVENTS.has(key)}`);
                        // H2: whitelist allowed event names — prevents crash via
                        // injected 'error' or 'newListener' events
                        if (key && ALLOWED_EVENTS.has(key)) {
                            this.events.emit(key, msg);
                        }
                    });
                } catch (e) {
                    if (this._log) this._log(`[NHC debug] parse error: ${e.message}`);
                }
            });
        });
